...
//               className="rounded-2xl bg-[#0C9AB0] cursor-pointer px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#0C9AB0] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-teal-600"
//             >
//               Send
//             </button>
//           </div>
...
export default function ChatModal({
  ...
  const [language] = useState("en-IN"); // Default language
  ...
  const startVoiceSearch = () => {
    ...
    recognition.onresult = (event) => {
      ...
      sendMessage(transcript); // Send directly
      setInput(""); // Clear input after voice search
    };
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          <div className="space-y-3">
            {messages.map((msg, i) => (
              <ChatMessage key={i} role={msg.role} content={msg.content} onRelatedClick={(q) => setInput(q)} />
            ))}
            {loading && <TypingIndicator />}
            {error && (
              <div className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">
        ...
        <form onSubmit={handleSubmit} className="shrink-0 border-t border-gray-200 bg-white p-4">
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`rounded-2xl bg-gray-200 px-3 py-2 text-sm cursor-pointer hover:bg-gray-300 transition-colors ${
